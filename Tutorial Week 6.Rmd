...
**Discussion:**
If we have a look at our final regression table, we can definitely observe that the $\beta_3$ coefficient - our interaction coefficient: After x Grow - changes substantially as we add covariates. The estimate decreases from 0.197 in the simple DiD (Model 1) to 0.131 after adding demographics (Model 2), and finally settles at 0.123 in the full model (Model 3). This reduction in the effect clearly suggests that our simple model was suffering from omitted variable bias, since it was attributing some of the effect of demographics and population to the coca boom. By adding all the covariates for the final model (Model 3), we isolate the true effect more accurately. Therefore, Model 3 seems to be the most credible specification - it includes all the covariates we identified as important and unbalanced at baseline. Our most believable estimate of the causal effect of the air-bridge disruption is an increase of 0.123 in the logged per-capita violent death rate, which is statistically significant at the 10% level. We can also see that the covariates are incredibly strong predictors. Just by adding **age** and **sex** in Model 2 causes the $R^2$ to jump from $0.9\%$ to $35\%$.
