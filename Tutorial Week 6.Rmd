...
3. Create `after` variable (1 if year ∈ {1996, 1997, 1998}, 0 otherwise)
4. Create `growafter` variable (grow × after)
...
2. Estimate: outcome = α + β·year + γ·grow + δ·(grow×year) + u
3. Test if grow×year interactions are jointly zero (year as linear and categorical)
...
3. Interpret placebo_after × grow coefficient
...
1. Estimate: outcome = β₀ + β₁·after + β₂·grow + β₃·(after×grow) + u
2. Report β₃ and p-value
3. Show analytically that β₃ equals the manual DiD estimate
...
[What does β₃ tell us about the causal effect?]
...
[Does β₃ change? Do covariates matter? Which specification is most credible?]
