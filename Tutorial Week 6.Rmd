...
```{r q10-did-regression, results='asis'}
# DiD regression model
did <- lm(outcome ~ after + grow + after:grow, data = dta_subset)

did_labels <- c(
  "After Period (1996-98)",
  "Coca-Growing Region",
  "After Period $\\times$ Growing Region",
  "Constant"
)
...
stargazer(
  did,
  type = "latex",
  title = "Table 4: Main Difference-in-Differences Estimate of Coca Boom on Violence",
  header = FALSE,
  column.labels = "DiD Model",
  dep.var.labels = "Logged Per-Capita Violent Death Rate",
  covariate.labels = did_labels,
  no.space = TRUE,
  omit.stat = c("adj.rsq", "ser", "f", "rsq")
)
```
...
We want to show that $\beta_3$ in the regression model represents the Difference-in-Differences (DiD) estimate.
Let $Y$ be the outcome, $G$ be the treatment group indicator ($G=1$ for `grow`, 0 otherwise), and $P$ be the post-treatment period indicator ($P=1$ for `after`, 0 otherwise).

The regression model is:
$$
Y = \beta_0 + \beta_1 P + \beta_2 G + \beta_3 (P \times G) + u
$$
The expected value of the outcome $Y$ conditional on the group and period is:
$$
E[Y | G, P] = \beta_0 + \beta_1 P + \beta_2 G + \beta_3 (P \times G)
$$
This gives us the expected value for each of the four groups:

* **Control, Before ($G=0, P=0$):** $E[Y | G=0, P=0] = \beta_0$
* **Control, After ($G=0, P=1$):** $E[Y | G=0, P=1] = \beta_0 + \beta_1$
* **Treatment, Before ($G=1, P=0$):** $E[Y | G=1, P=0] = \beta_0 + \beta_2$
* **Treatment, After ($G=1, P=1$):** $E[Y | G=1, P=1] = \beta_0 + \beta_1 + \beta_2 + \beta_3$

The DiD parameter, let's call it $\delta$, is defined as the difference in the "after-before" change for the treatment group and the "after-before" change for the control group:
...
```{r q11-comparison, results='asis'}
# Compare models
stargazer(cov_did1, cov_did2, cov_did3,     
          type = "latex",                 
...
**Discussion:**
Looking at our final regression table, we can see that the $\beta_3$ coefficient - our interaction coefficient: After x Grow - changes substantially as we add covariates. The estimate decreases from 0.197 in the simple difference-in-difference (DiD) model (Model 1) to 0.131 after demographics are added (Model 2), finally settling at 0.123 in the full model (Model 3). This reduction in the effect clearly suggests that our simple model was subject to omitted variable bias, as it attributed some of the effect of demographics and population to the coca boom. By adding all the covariates to the final model (Model 3), we can isolate the true effect more accurately. Therefore, Model 3 seems to be the most credible specification, as it includes all the covariates that we identified as important and imbalanced at baseline. Our most reliable estimate of the causal effect of the air-bridge disruption is an increase of 0.123 in the logged per capita violent death rate, which is statistically significant at the 10% level. We can also see that the covariates are incredibly strong predictors. Simply by adding **age** and **sex** in Model 2 causes the $R^2$ to jump from $0.9\%$ to $35\%$, and $36.3\%$ in the final model.
...
* Based on our evidence we can definitely say that violence increased in the coca-growing regions as a result of the air-bridge disruption. Firstly, our initial density plots from Q2 and Q3 showed a clear rightward shift in the violence distribution for men in the growing regions after the disruption. The age-specific plots further revealed that this impact was concentrated among young adolescents and young adults. Our main DiD regression analysis confirmed this visual finding. Our most credible estimate (from Model 3 in Q11) showed a statistically significant effect. The Interaction: After x Grow coefficient ($\beta_3$) of 0.123 (p < 0.1) indicates that the coca boom caused a real increase in the logged per-capita violent death rate, even after controlling for demographic and population differences. 

2. Does the evidence support a "resource-curse" interpretation?
* The 'resource curse' is the idea that a sudden increase in natural resource wealth can paradoxically lead to more conflict and violence, rather than less. The disruption to the air bridge created a sudden, massive increase in the profitability of coca in Colombia. Our analysis demonstrates that this economic boom did not lead to prosperity, but rather to a statistically significant increase in violence, likely resulting from armed groups fighting to control the newly profitable territory. This leads us to conclude that our evidence supports a 'resource-curse' interpretation. 

3. What are the remaining identification threats?
* Although our analysis successfully confirmed parallel trends and controlled for baseline confounders, several potential threats to perfect causal identification remain. The most significant of these is the possibility of unobserved, time-varying confounders. The validity of our DiD model rests on the assumption that no other event occurred at the same time (1996–98) that only affected the coca-growing regions. For example, if the government had launched a separate military campaign in those rural areas at the same time, our model might incorrectly attribute the resulting violence to the coca boom. Furthermore, our model assumes that there were no spillover effects, meaning that the control group (grow=0) was completely unaffected by the boom. However, if the massive increase in violence in the growing regions 'spilled over' into the non-growing regions, perhaps through refugee flows or spreading conflict, our control group would be 'contaminated'. Lastly, we must acknowledge the potential for measurement error. Our entire analysis relies on violent death data being accurately and consistently reported across all regions and years. If data reporting improved in rural areas after 1996 for some reason, this would appear as an increase in violence, even though it was actually just an increase in reporting.
